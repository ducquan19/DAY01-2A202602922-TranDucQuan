...
def format_comparison_table(results: list[dict]) -> str:
    ...
    def truncate(text: str, max_len: int = 40) -> str:
        ...
        return text[:max_len-3] + "..." if len(text) > max_len else text.ljust(max_len)
    ...
    for r in results:
        p = truncate(r["prompt"])
        g_ans = truncate(r["gpt4o_answer"])
        m_ans = truncate(r["mini_answer"])
        g_time = f"{r['gpt4o_time']:.2f}s".ljust(15)
        m_time = f"{r['mini_time']:.2f}s".ljust(15)
        lines.append(f"{p} | {g_ans} | {m_ans} | {g_time} | {m_time}")
